...
#define PERIOD 0.1
#define CNT_PNTS 200
#define RAD_FREQ 0.3141592653	// 2*PI*0.05
...
int main(int argc, char *argv[]) {
    ...
    std::array<double,7> q_drag = {{0,PI/6,0,PI/3,0,PI/2,0}};	// scaling factor: 0.3
	...
	for (int cnt = 0; cnt <= CNT_PNTS; cnt++){
		...
		setpoint = coeffs * fourier;		// scaling factor: 0.3
    ...
    joint_position_callback = [&](RCI::robot::RobotState robot_state) -> JointPositions {
		...
		// S-curve planner for motion planning, speed factor: 0.4
        JointMotionGenerator joint_s(0.4, joint_motion_vector[vec_cnt]);
		...
		if (joint_s.calculateDesiredValues_joint(time, joint_delta) == false) {
			...
			// time's up
            if (vec_cnt >= joint_motion_vector.size()) {
